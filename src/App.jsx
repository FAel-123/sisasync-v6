                    ...
                    {activeTab === 'analysis' && (
                        <div className="space-y-6 animate-in slide-in-from-bottom-4 duration-500">
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <div className={`p-5 rounded-2xl border ${isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}`}><p className="text-xs font-bold opacity-60 uppercase mb-1">Total Waste</p><h3 className="text-3xl font-black text-emerald-500">{totalWasteWeight.toFixed(1)} <span className="text-sm text-slate-400">kg</span></h3></div>
                                <div className={`p-5 rounded-2xl border ${isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}`}><p className="text-xs font-bold opacity-60 uppercase mb-1">Active Recyclers</p><h3 className="text-3xl font-black text-blue-500">{Object.keys(userLeaderboard).length} <span className="text-sm text-slate-400">students</span></h3></div>
                                <div className={`p-5 rounded-2xl border ${isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}`}><p className="text-xs font-bold opacity-60 uppercase mb-1">Engagement</p><h3 className="text-3xl font-black text-orange-500">{Math.round((Object.keys(userLeaderboard).length / Math.max(totalUsers, 1)) * 100)}% <span className="text-sm text-slate-400">active</span></h3></div>
                            </div>

                            {/* ACTIVITY TRENDS CHART (FIXED DATE) */}
                            <div className={`p-6 rounded-2xl border shadow-sm ${isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}`}>
                                <h4 className="font-bold mb-6 flex items-center gap-2"><TrendingUp size={18} className="text-blue-500"/> Daily Activity (Last 7 Days)</h4>
                                <div className="h-40 flex items-end gap-2 md:gap-4">
                                    {activityData.map((d, i) => (
                                        <div key={i} className="flex-1 flex flex-col items-center gap-2 group relative">
                                            <div className="text-[10px] font-bold opacity-0 group-hover:opacity-100 transition-opacity absolute -top-6">{d.count}</div>
                                            <div className={`w-full rounded-t-lg transition-all duration-500 ${d.count > 0 ? 'bg-blue-500 hover:bg-blue-400' : 'bg-slate-200'}`} style={{height: `${d.count > 0 ? (d.count / maxActivity) * 100 : 2}%`}}></div>
                                            <span className="text-[10px] font-bold opacity-60">{d.date.slice(5)}</span>
                                        </div>
                                    ))}
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                <div className={`p-6 rounded-2xl border shadow-sm ${isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}`}>
                                    <h4 className="font-bold mb-6 flex items-center gap-2"><BarChart3 size={18} className="text-emerald-500"/> Waste Breakdown</h4>
                                    <div className="space-y-4">
                                        {Object.entries(categoryStats).map(([cat, weight]) => {
                                            const percent = totalWasteWeight > 0 ? Math.round((weight / totalWasteWeight) * 100) : 0;
                                            return (<div key={cat}><div className="flex justify-between text-xs font-bold mb-1"><span>{cat}</span><span>{weight.toFixed(1)} kg ({percent}%)</span></div><div className="w-full bg-slate-200 rounded-full h-2.5 overflow-hidden"><div className={`h-full rounded-full ${cat === 'Plastic' ? 'bg-blue-500' : cat === 'Paper' ? 'bg-yellow-500' : cat === 'Tin' ? 'bg-slate-500' : 'bg-purple-500'}`} style={{width: `${percent}%`}}></div></div></div>)
                                        })}
                                    </div>
                                    <div className="mt-6 p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/20"><p className="text-xs font-bold text-emerald-600 flex items-center gap-2"><Zap size={14}/> Insight:</p><p className="text-xs opacity-70 mt-1">Plastic waste dominates the collection. Consider launching a "Plastic-Free Week" campaign.</p></div>
                                </div>
