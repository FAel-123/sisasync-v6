        ...
        <div className={`min-h-screen flex font-sans ${isDark ? 'bg-slate-900 text-slate-100' : 'bg-slate-50 text-slate-800'}`}>
            ...
            <main className={`flex-1 p-4 md:p-10 transition-all duration-300 xl:ml-64 w-full`}>
                ...
                <div className="w-full max-w-[100vw] overflow-hidden">
                    ...
                    <div><label className={`block text-xs font-bold mb-2 uppercase ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Total Volunteers Needed</label><input type="number" value={eventForm.volunteers_needed} onChange={e=>setEventForm({...eventForm, volunteers_needed: e.target.value})} placeholder="e.g 20" required className={`w-full border p-3 rounded-xl font-bold text-sm outline-none focus:ring-2 focus:ring-emerald-500 ${isDark ? 'bg-slate-900 border-slate-700' : 'bg-slate-50 border-slate-200'}`}/><p className="text-[10px] mt-1 opacity-60">This number will be split among 4 roles (Logistics, Multimedia, Protocol, Promo).</p></div>
