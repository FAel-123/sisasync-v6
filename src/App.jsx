...
const VOLUNTEER_ROLES = [
    { id: 'logistic', name: 'Logistics & Setup', icon: Wrench, color: 'bg-orange-500' },
    { id: 'multimedia', name: 'Multimedia & Photo', icon: Camera, color: 'bg-purple-500' },
    { id: 'protocol', name: 'Protocol & Emcee', icon: Mic, color: 'bg-blue-500' },
    { id: 'promo', name: 'Promotion & Socials', icon: Megaphone, color: 'bg-pink-500' }
];

// --- DEFAULT VOTING LOCATIONS ---
...
const NotFoundPage = () => {
    ...
    return (
        <div className="min-h-screen bg-emerald-600 flex flex-col items-center justify-center text-white p-6 text-center relative overflow-hidden">
            ...
            <button onClick={() => navigate('/')} className="bg-white text-emerald-700 px-8 py-3 rounded-full font-bold shadow-lg hover:scale-105 transition-transform flex items-center gap-2"><Home size={20}/> Back to Home</button>
        </div>
    );
};
...
const RedeemModal = ({ isOpen, onClose, reward, userPoints, onConfirm, isDark }) => {
    ...
    const handleConfirm = () => { setIsProcessing(true); onConfirm(reward); setTimeout(() => setIsProcessing(false), 2000); };
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm animate-in fade-in duration-200 px-4">
...
const VolunteerModal = ({ isOpen, onClose, event, onConfirm, isDark }) => {
    ...
    useEffect(() => { if (isOpen && event) fetchCounts(); }, [isOpen, event]);
    ...
    const totalNeeded = event.volunteers_needed || 20; 
    const limitPerRole = Math.ceil(totalNeeded / 4);
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm animate-in fade-in px-4">
            <div className={`w-full max-w-md p-6 rounded-3xl shadow-2xl relative ${isDark ? 'bg-slate-900 text-white border border-slate-700' : 'bg-white text-slate-900'}`}>
                <button onClick={onClose} className="absolute top-4 right-4 opacity-50 hover:opacity-100"><X/></button>
                <div className="mb-6"><h3 className="text-xl font-black mb-1">Volunteer Registration</h3><p className="text-xs opacity-60">Event: {event.title}</p><p className="text-xs text-emerald-500 font-bold mt-1">Target: {totalNeeded} Volunteers</p></div>
                <p className="text-sm font-bold mb-3">Select Role:</p>
                {loadingCounts ? <div className="text-center py-4"><Loader2 className="animate-spin mx-auto"/></div> : (
                    <div className="grid grid-cols-1 gap-3 mb-6">
                        {VOLUNTEER_ROLES.map(role => {
                            ...
                            const isFull = currentCount >= limitPerRole;
                            return (
                                <div key={role.id} onClick={() => !isFull && setSelectedRole(role.id)} className={`p-3 rounded-xl border-2 transition-all flex items-center justify-between gap-3 ${isFull ? 'opacity-50 cursor-not-allowed bg-slate-100 border-slate-200 grayscale' : 'cursor-pointer'} ${selectedRole === role.id ? 'border-emerald-500 bg-emerald-50 ring-1 ring-emerald-500' : isDark ? 'border-slate-700 hover:bg-slate-800' : 'border-slate-100 hover:bg-slate-50'}`}>
                                    <div className="flex items-center gap-3"><div className={`${role.color} p-2 rounded-full text-white shadow-sm`}><role.icon size={16}/></div><div><span className="text-xs font-bold block">{role.name}</span><span className={`text-[10px] font-bold ${isFull ? 'text-red-500' : 'text-emerald-600'}`}>{isFull ? "FULL" : `${limitPerRole - currentCount} slots left`}</span></div></div>
                                    <div className="w-16 h-1.5 bg-slate-200 rounded-full overflow-hidden"><div className={`h-full ${isFull ? 'bg-red-500' : 'bg-emerald-500'}`} style={{width: `${(currentCount / limitPerRole) * 100}%`}}></div></div>
                                </div>
                            );
                        })}
                    </div>
...
const EcoCubeModal = ({ isOpen, onClose, isDark, topLocations }) => {
    if (!isOpen) return null;
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm animate-in fade-in px-4">
            <div className={`w-full max-w-lg p-6 rounded-3xl shadow-2xl relative ${isDark ? 'bg-slate-900 text-white border border-slate-700' : 'bg-white text-slate-900'}`}>
                ...
                <h4 className="font-bold mb-3 flex items-center gap-2"><Clock size={16}/> Next Pickup Locations (Based on Vote)</h4>
                <div className={`rounded-xl border overflow-hidden mb-6 ${isDark ? 'border-slate-700' : 'border-slate-200'}`}>
                    <table className="w-full text-sm text-left">
                        <thead className={isDark ? 'bg-slate-800' : 'bg-slate-50'}><tr><th className="p-3">Rank</th><th className="p-3">Location</th><th className="p-3">Schedule</th></tr></thead>
                        <tbody className="divide-y opacity-90">
                            {topLocations && topLocations.length > 0 ? topLocations.map((loc, idx) => (
                                <tr key={idx}><td className="p-3 font-bold">#{idx+1}</td><td className="p-3">{loc.name}</td><td className="p-3 text-emerald-500 font-bold">{idx===0 ? 'Mon & Thu' : idx===1 ? 'Tue & Fri' : 'Wednesday'}</td></tr>
                            )) : (
                                <tr><td colSpan="3" className="p-6 text-center opacity-50">Voting in progress. Locations TBD.</td></tr>
                            )}
                        </tbody>
                    </table>
                </div>
...
export default function App() {
  ...
  const [votingSession, setVotingSession] = useState({
      isActive: true,
      endDate: '2026-01-30', // Default Deadline
  ...
  useEffect(() => {
    ...
    const savedVoting = localStorage.getItem('educycle_voting');
    
    if (savedUser) setCurrentUser(JSON.parse(savedUser));
    if (savedLang) setLanguage(savedLang);
    if (savedTheme === 'dark') setIsDark(true);
    if (savedVoting) setVotingSession(JSON.parse(savedVoting));
  }, []);
  ...
  useEffect(() => {
      localStorage.setItem('educycle_voting', JSON.stringify(votingSession));
  }, [votingSession]);
  ...
  return (
    <Router>
      ...
      <AppRoutes currentUser={currentUser} setCurrentUser={handleSetUser} votingSession={votingSession} setVotingSession={setVotingSession} {...globalProps} />
    </Router>
  );
}

function AppRoutes({ currentUser, setCurrentUser, votingSession, setVotingSession, language, setLanguage, isDark, setIsDark, t }) {
  ...
  const refreshData = async () => {
    ...
    const DUMMY_REQUESTS = [
        { id: 101, student: "Ali Bin Abu", type: "Plastic", weight: "5.5", points: 55, status: "Approved", date: getLocalISOString(0), method: "Pickup", location: "Hostel Block A" },
        { id: 102, student: "Mei Ling", type: "Paper", weight: "12.0", points: 60, status: "Approved", date: getLocalISOString(0), method: "Drop-off", location: "Library" },
        { id: 103, student: "Raju A/L Muthu", type: "Tin", weight: "3.2", points: 48, status: "Approved", date: getLocalISOString(1), method: "Pickup", location: "Cafe" },
        { id: 104, student: "Sarah Tan", type: "E-Waste", weight: "1.5", points: 150, status: "Approved", date: getLocalISOString(1), method: "Drop-off", location: "Faculty Eng" },
        { id: 105, student: "Ali Bin Abu", type: "Plastic", weight: "4.0", points: 40, status: "Approved", date: getLocalISOString(2), method: "Pickup", location: "Hostel Block A" },
        { id: 106, student: "Zaim Haziq", type: "Paper", weight: "8.5", points: 42, status: "Approved", date: getLocalISOString(3), method: "Pickup", location: "Sports Complex" },
        { id: 107, student: "Mei Ling", type: "Plastic", weight: "6.0", points: 60, status: "Approved", date: getLocalISOString(4), method: "Drop-off", location: "Library" },
        { id: 108, student: "Raju A/L Muthu", type: "Tin", weight: "2.0", points: 30, status: "Approved", date: getLocalISOString(5), method: "Pickup", location: "Cafe" },
        { id: 109, student: "Admin Tester", type: "E-Waste", weight: "5.0", points: 500, status: "Approved", date: getLocalISOString(6), method: "Drop-off", location: "Admin Office" },
    ];
    ...
    // GABUNGKAN DATA
    const mergedRequests = [...(reqs.data || []), ...DUMMY_REQUESTS];
    setDbRequests(mergedRequests);
  ...
  const isVotingExpired = today > votingSession.endDate;
  const winningLocations = isVotingExpired 
      ? [...votingSession.locations].sort((a, b) => b.votes - a.votes).slice(0, 3) 
      : [];

  const props = { currentUser, setCurrentUser, dbRequests, dbEvents, dbNews, dbRedemptions, dbProducts, fetchRequests: refreshData, companyFund, fundGoal, language, setLanguage, isDark, setIsDark, t, joinedEventIds, votingSession, setVotingSession, winningLocations, isVotingExpired };
...
// --- MEMBER ZONE PAGE ---
function MemberZone({ currentUser, dbEvents, isDark, votingSession, setVotingSession, isVotingExpired, winningLocations }) {
    const navigate = useNavigate();
    ...
    const handleVote = (id) => {
        if(isVotingExpired) return setToast({message: "Voting period has ended!", type: "error"});
        if(hasVoted) return setToast({message: "You have already voted!", type: "error"});
        
        const updatedLocs = votingSession.locations.map(l => l.id === id ? {...l, votes: l.votes + 1} : l);
        setVotingSession({...votingSession, locations: updatedLocs});
        setHasVoted(true);
        setToast({message: "Vote cast successfully!", type: "success"});
    };

    const totalVotes = votingSession.locations.reduce((acc, curr) => acc + curr.votes, 0);

    return (
        <div className={`min-h-screen font-sans pb-20 ${isDark ? 'bg-slate-900 text-slate-100' : 'bg-slate-50 text-slate-800'}`}>
            {toast && <Toast message={toast.message} type={toast.type} onClose={() => setToast(null)} />}
            <VolunteerModal isOpen={!!selectedEvent} onClose={() => setSelectedEvent(null)} event={selectedEvent} onConfirm={handleVolunteer} isDark={isDark}/>
            ...
            <main className="max-w-4xl mx-auto p-6">
                <div className="w-full max-w-md mx-auto aspect-[1.586/1] rounded-3xl bg-gradient-to-br from-yellow-400 via-orange-500 to-red-500 p-6 text-white shadow-2xl relative overflow-hidden mb-10 transform hover:scale-105 transition-transform duration-500">
                    ...
                    <div className="flex justify-between items-start mb-8 relative z-10"><div className="flex items-center gap-2"><BookOpen className="w-6 h-6 text-white"/><span className="font-black tracking-widest text-lg">EDUCYCLE</span></div><span className="bg-white/20 backdrop-blur-md px-3 py-1 rounded-lg text-xs font-bold border border-white/30">PREMIUM MEMBER</span></div>
                    <div className="mt-auto relative z-10"><div className="flex justify-between items-end"><div><p className="text-xs uppercase opacity-80 mb-1 tracking-wider">Member Name</p><h2 className="text-2xl font-black tracking-wide text-shadow">{currentUser?.name?.toUpperCase()}</h2></div><QrCode size={48} className="opacity-80"/></div><p className="mt-4 text-xs font-mono opacity-70">ID: {currentUser?.memberId || "Generating..."}</p></div>
                </div>
                    ...
                    <div className={`p-6 rounded-2xl border ${isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}`}>
                        {isVotingExpired ? (
                            <div className="text-center py-6">
                                <h4 className="text-2xl font-black text-emerald-500 mb-2">Voting Ended!</h4>
                                <p className="text-sm opacity-60 mb-6">Here are the winning locations for next week:</p>
                                <div className="space-y-3">
                                    {winningLocations.map((loc, idx) => (
                                        <div key={idx} className="flex items-center justify-between p-3 rounded-xl bg-emerald-500/10 border border-emerald-500/30">
                                            <span className="font-bold flex items-center gap-2"><span className="bg-emerald-500 text-white w-6 h-6 rounded-full flex items-center justify-center text-xs">#{idx+1}</span> {loc.name}</span>
                                            <span className="font-black text-emerald-600">{loc.votes} votes</span>
                                        </div>
                                    ))}
                                </div>
                            </div>
                            ...
                            <>
                                <div className="flex justify-between items-center mb-4">
                                    <p className="text-sm opacity-60">Cast your vote for the next Eco-Cube spot!</p>
                                    <span className="text-xs font-bold bg-red-100 text-red-600 px-2 py-1 rounded">Ends: {votingSession.endDate}</span>
                                </div>
                                <div className="space-y-4">
                                    {votingSession.locations.map(loc => {
                                        const percent = totalVotes > 0 ? Math.round((loc.votes / totalVotes) * 100) : 0;
                                        return (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {dbEvents.map(ev => (
                        <div key={ev.id} className={`p-5 rounded-2xl border transition-all hover:shadow-lg ${isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}`}>
                            ...
                            <div className="mb-4 bg-emerald-500/10 p-2 rounded-lg text-xs text-emerald-600 font-bold text-center border border-emerald-500/20">
                                Wanted: {ev.volunteers_needed || 20} Volunteers
                            </div>
       ...
       {showAdminModal && (
           <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm animate-in fade-in px-4">
               <div className={`bg-slate-900 w-full max-w-sm p-8 rounded-3xl shadow-2xl border border-slate-700 relative ${isShaking ? 'animate-shake' : ''}`}>
                   ...
                   <div className="mb-6">
                       <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-2 block text-center">Enter Passcode</label>
...
// 3. USER DASHBOARD
function UserDashboard({ currentUser, setCurrentUser, dbRequests, dbEvents, dbNews, dbRedemptions, dbProducts, fetchRequests, companyFund, fundGoal, t, language, setLanguage, isDark, setIsDark, joinedEventIds, winningLocations }) {
  const navigate = useNavigate();
...
function AdminDashboard({ currentUser, setCurrentUser, dbRequests, dbEvents, dbNews, dbProducts, fetchData, companyFund, t, language, setLanguage, isDark, setIsDark, votingSession, setVotingSession }) {
    ...
    const [eventForm, setEventForm] = useState({ title: '', date: '', loc: '', img: null, zoom_enabled: false, volunteers_needed: 20 });
    ...
    const handleAddEvent = async (e) => { e.preventDefault(); if (!eventForm.img) return setToast({message: "Please upload image!", type: "error"}); await supabase.from('events').insert([eventForm]); setToast({message: "Event Published!", type: "success"}); fetchData(); setEventForm({ title: '', date: '', loc: '', img: null, zoom_enabled: false, volunteers_needed: 20 }); };
    ...
    const handleAddLocation = (e) => {
        ...
        const newId = votingSession.locations.length + 1;
    ...
    const handleUpdateDeadline = (e) => {
        setVotingSession({ ...votingSession, endDate: e.target.value });
        setToast({ message: "Deadline updated!", type: "success" });
    };

    const handleResetVotes = () => {
        if(window.confirm("Are you sure? This will reset all votes to 0.")) {
            const resetLocs = votingSession.locations.map(l => ({ ...l, votes: 0 }));
            setVotingSession({ ...votingSession, locations: resetLocs });
            setToast({ message: "Votes reset!", type: "success" });
        }
    };
    ...
    return (
        <div className={`min-h-screen flex font-sans ${isDark ? 'bg-slate-900 text-slate-100' : 'bg-slate-50 text-slate-800'}`}>
            ...
            {/* OVERLAY (Updated Breakpoint to xl) */}
            {isSidebarOpen && (<div className="fixed inset-0 z-40 bg-black/60 backdrop-blur-sm xl:hidden animate-in fade-in" onClick={() => setIsSidebarOpen(false)}></div>)}

            {/* SIDEBAR (Updated Breakpoint to xl) */}
            <aside className={`fixed inset-y-0 left-0 z-50 w-64 transform transition-transform duration-300 ease-in-out ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} xl:translate-x-0 ${isDark ? 'bg-slate-950 text-slate-400 border-r border-slate-800' : 'bg-slate-900 text-slate-400'} flex flex-col shadow-2xl xl:shadow-none`}>
                <div className="p-6 text-white font-black text-2xl flex items-center justify-between">
                    ...
                    <button onClick={() => setIsSidebarOpen(false)} className="xl:hidden p-1 rounded-lg hover:bg-white/10 text-slate-400 hover:text-white transition-colors"><X size={24}/></button>
                </div>
                ...
                <nav className="flex-1 px-4 space-y-2 overflow-y-auto">
                    ...
                    <button onClick={() => { setActiveTab('analysis'); setIsSidebarOpen(false); }} className={`w-full flex items-center gap-3 p-3 rounded-xl font-bold text-sm transition-all ${activeTab === 'analysis' ? 'bg-emerald-600 text-white' : 'hover:bg-slate-800'}`}><BarChart3 size={18}/> User Analysis</button>
                    <button onClick={() => { setActiveTab('voting'); setIsSidebarOpen(false); }} className={`w-full flex items-center gap-3 p-3 rounded-xl font-bold text-sm transition-all ${activeTab === 'voting' ? 'bg-emerald-600 text-white' : 'hover:bg-slate-800'}`}><Vote size={18}/> Manage Voting</button>
            ...
            {/* MAIN CONTENT (Updated Breakpoint to xl) */}
            <main className={`flex-1 p-4 md:p-10 transition-all duration-300 xl:ml-64 w-full`}>
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
                    <div className="flex items-center gap-3">
                        {/* BUTTON VISIBLE UNTIL XL SCREEN (1280px) */}
                        <button onClick={() => setIsSidebarOpen(true)} className="xl:hidden p-2 rounded-xl bg-emerald-600 text-white hover:bg-emerald-700 shadow-lg active:scale-95 transition-all"><Menu size={24}/></button>
                        <h2 className="text-2xl md:text-3xl font-black truncate">{activeTab === 'overview' ? t.adminPanel : activeTab === 'voting' ? 'Manage Voting' : activeTab === 'analysis' ? 'User Analysis' : activeTab === 'requests' ? t.pickup : activeTab === 'events' ? t.manageEvents : activeTab === 'news' ? t.updates : 'Merchandise'}</h2>
                    </div>
                ...
                <div className="w-full max-w-[100vw] overflow-hidden">
                    ...
                    {activeTab === 'analysis' && (<div className="space-y-6 animate-in slide-in-from-bottom-4 duration-500"><div className="grid grid-cols-1 md:grid-cols-3 gap-4"><div className={`p-5 rounded-2xl border ${isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}`}><p className="text-xs font-bold opacity-60 uppercase mb-1">Total Waste</p><h3 className="text-3xl font-black text-emerald-500">{totalWasteWeight.toFixed(1)} <span className="text-sm text-slate-400">kg</span></h3></div><div className={`p-5 rounded-2xl border ${isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}`}><p className="text-xs font-bold opacity-60 uppercase mb-1">Active Recyclers</p><h3 className="text-3xl font-black text-blue-500">{Object.keys(userLeaderboard).length} <span className="text-sm text-slate-400">students</span></h3></div><div className={`p-5 rounded-2xl border ${isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}`}><p className="text-xs font-bold opacity-60 uppercase mb-1">Engagement</p><h3 className="text-3xl font-black text-orange-500">{Math.round((Object.keys(userLeaderboard).length / Math.max(totalUsers, 1)) * 100)}% <span className="text-sm text-slate-400">active</span></h3></div></div><div className={`p-6 rounded-2xl border shadow-sm ${isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}`}><h4 className="font-bold mb-6 flex items-center gap-2"><TrendingUp size={18} className="text-blue-500"/> Daily Activity (Last 7 Days)</h4><div className="h-40 flex items-end gap-2 md:gap-4">{activityData.map((d, i) => (<div key={i} className="flex-1 flex flex-col items-center gap-2 group relative"><div className="text-[10px] font-bold opacity-0 group-hover:opacity-100 transition-opacity absolute -top-6">{d.count}</div><div className={`w-full rounded-t-lg transition-all duration-500 ${d.count > 0 ? 'bg-blue-500 hover:bg-blue-400' : 'bg-slate-200'}`} style={{height: `${d.count > 0 ? (d.count / maxActivity) * 100 : 2}%`}}></div><span className="text-[10px] font-bold opacity-60">{d.date.slice(5)}</span></div>))}</div></div><div className="grid grid-cols-1 lg:grid-cols-2 gap-6"><div className={`p-6 rounded-2xl border shadow-sm ${isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}`}><h4 className="font-bold mb-6 flex items-center gap-2"><BarChart3 size={18} className="text-emerald-500"/> Waste Breakdown</h4><div className="space-y-4">{Object.entries(categoryStats).map(([cat, weight]) => { const percent = totalWasteWeight > 0 ? Math.round((weight / totalWasteWeight) * 100) : 0; return (<div key={cat}><div className="flex justify-between text-xs font-bold mb-1"><span>{cat}</span><span>{weight.toFixed(1)} kg ({percent}%)</span></div><div className="w-full bg-slate-200 rounded-full h-2.5 overflow-hidden"><div className={`h-full rounded-full ${cat === 'Plastic' ? 'bg-blue-500' : cat === 'Paper' ? 'bg-yellow-500' : cat === 'Tin' ? 'bg-slate-500' : 'bg-purple-500'}`} style={{width: `${percent}%`}}></div></div></div>) })}</div></div><div className={`p-6 rounded-2xl border shadow-sm ${isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}`}><h4 className="font-bold mb-6 flex items-center gap-2"><Star size={18} className="text-yellow-500 fill-yellow-500"/> Top 5 Eco-Warriors</h4><div className="space-y-4">{topUsersList.length === 0 ? <p className="text-sm opacity-50 italic">No data yet.</p> : topUsersList.map((u, idx) => (<div key={idx} className={`flex items-center justify-between p-3 rounded-xl border ${idx === 0 ? 'bg-yellow-500/10 border-yellow-500/30' : isDark ? 'bg-slate-900 border-slate-700' : 'bg-slate-50 border-slate-100'}`}><div className="flex items-center gap-3"><div className={`w-8 h-8 rounded-full flex items-center justify-center font-black text-sm ${idx === 0 ? 'bg-yellow-500 text-white shadow-lg shadow-yellow-500/40' : idx === 1 ? 'bg-slate-400 text-white' : idx === 2 ? 'bg-orange-700 text-white' : 'bg-slate-200 text-slate-500'}`}>{idx + 1}</div><div><p className="font-bold text-sm">{u.name}</p><p className="text-[10px] opacity-60">{u.count} contributions</p></div></div><div className="text-right"><p className="font-black text-emerald-500 text-sm">{u.points} pts</p><p className="text-[10px] opacity-60">{u.weight.toFixed(1)} kg</p></div></div>))}</div></div></div></div>)}
                    ...
                    {activeTab === 'merch' && (<div className="animate-in slide-in-from-bottom-4 duration-500"><div className={`p-6 rounded-2xl shadow-sm border mb-8 ${isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}`}><div className="flex justify-between items-center mb-4"><h4 className="font-bold">Add New Product</h4><div className="w-1/3"><div className="flex justify-between text-[10px] mb-1 font-bold"><span className="text-emerald-500">Sales Progress</span><span>{totalSales} items sold</span></div><div className="w-full bg-slate-200 rounded-full h-2 overflow-hidden"><div className="bg-emerald-500 h-full rounded-full" style={{width: `${Math.min(totalSales, 100)}%`}}></div></div></div></div><form onSubmit={handleAddProduct} className="grid grid-cols-1 md:grid-cols-2 gap-4"><input value={productForm.name} onChange={e=>setProductForm({...productForm, name: e.target.value})} placeholder="Product Name" required className={`border p-3 rounded-xl font-bold text-sm outline-none focus:ring-2 focus:ring-emerald-500 ${isDark ? 'bg-slate-900 border-slate-700' : 'bg-slate-50 border-slate-200'}`}/><input value={productForm.price} onChange={e=>setProductForm({...productForm, price: e.target.value})} placeholder="Price (e.g. RM 15 / 500 pts)" required className={`border p-3 rounded-xl font-bold text-sm outline-none focus:ring-2 focus:ring-emerald-500 ${isDark ? 'bg-slate-900 border-slate-700' : 'bg-slate-50 border-slate-200'}`}/><div className="md:col-span-2"><label className={`block text-xs font-bold mb-2 uppercase ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Product Image</label><div className={`relative border-2 border-dashed rounded-xl h-32 overflow-hidden transition-colors ${isDark ? 'border-slate-700 hover:border-emerald-500' : 'border-slate-300 hover:border-emerald-500'}`}><input type="file" accept="image/*" onChange={(e)=>handleImageUpload(e, 'product')} className="hidden" id="prodUpload"/><label htmlFor="prodUpload" className="absolute inset-0 w-full h-full flex flex-col items-center justify-center cursor-pointer z-10">{productForm.img ? <img src={productForm.img} className="w-full h-full object-cover"/> : <div className="text-center text-xs opacity-50"><Upload size={20} className="mx-auto mb-1"/>Upload Photo</div>}</label></div></div><button className="md:col-span-2 bg-emerald-600 text-white py-3 rounded-xl font-bold hover:bg-emerald-700">List Item</button></form></div><div className="grid grid-cols-2 md:grid-cols-4 gap-4">{dbProducts.map(p => (<div key={p.id} className={`relative group p-3 rounded-xl border flex flex-col transition-all ${deletingIds.includes(p.id) ? 'opacity-0 scale-95' : 'opacity-100'} ${isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200 shadow-sm'}`}><button onClick={() => setDeleteTarget({ id: p.id, type: 'product' })} className="absolute top-2 right-2 z-10 bg-red-500 text-white p-1.5 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity shadow-lg"><Trash2 size={14}/></button><div className="h-32 bg-slate-200 rounded-lg mb-3 overflow-hidden"><img src={p.img} className="w-full h-full object-cover"/></div><h4 className="font-bold text-sm truncate">{p.name}</h4><div className="flex justify-between items-center mt-auto pt-2"><span className="text-xs opacity-60">{p.sold || 0} sold</span><span className="text-emerald-500 font-black text-sm">{p.price}</span></div></div>))}</div></div>)}
                    {/* --- NEW VOTING MANAGEMENT TAB --- */}
                    {activeTab === 'voting' && (
                        <div className="animate-in slide-in-from-bottom-4 duration-500">
                            <div className={`p-6 rounded-2xl shadow-sm border mb-8 ${isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}`}>
                                <h4 className="font-bold mb-4 flex items-center gap-2"><Vote size={20}/> Manage Voting Session</h4>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                                    <div>
                                        <label className="text-xs font-bold opacity-60 mb-2 block uppercase">Set Voting Deadline</label>
                                        <div className="flex gap-2">
                                            <input type="date" value={votingSession.endDate} onChange={handleUpdateDeadline} className={`border p-3 rounded-xl font-bold text-sm outline-none flex-1 ${isDark ? 'bg-slate-900 border-slate-700' : 'bg-slate-50 border-slate-200'}`} />
                                        </div>
                                    </div>
                                    <div>
                                        <label className="text-xs font-bold opacity-60 mb-2 block uppercase">Add New Location</label>
                                        <form onSubmit={handleAddLocation} className="flex gap-2">
                                            <input value={newLocation} onChange={(e)=>setNewLocation(e.target.value)} placeholder="Location Name" className={`border p-3 rounded-xl font-bold text-sm outline-none flex-1 ${isDark ? 'bg-slate-900 border-slate-700' : 'bg-slate-50 border-slate-200'}`} />
                                            <button className="bg-emerald-600 text-white p-3 rounded-xl hover:bg-emerald-700"><Plus/></button>
                                        </form>
                                    </div>
                                </div>
                                <div className="flex justify-between items-center mb-4"><h5 className="font-bold text-sm">Current Locations & Votes</h5><button onClick={handleResetVotes} className="text-xs text-red-500 font-bold hover:underline">Reset All Votes</button></div>
                                <div className="space-y-3">
                                    {votingSession.locations.map((loc) => (
                                        <div key={loc.id} className={`flex justify-between items-center p-3 rounded-xl border ${isDark ? 'bg-slate-900 border-slate-700' : 'bg-slate-50 border-slate-100'}`}>
                                            <div className="flex items-center gap-3">
                                                <div className={`w-3 h-3 rounded-full ${loc.color}`}></div>
                                                <span className="font-bold text-sm">{loc.name}</span>
                                            </div>
                                            <span className="font-mono font-bold">{loc.votes} votes</span>
                                        </div>
                                    ))}
                                </div>
                            </div>
